// Dev path-based routing for article pages.
// In production, subdomain routing uses (publication)/[articleSlug]/page.tsx instead.
// Note: outer [articleSlug] param = publication slug, inner [slug] param = article slug.
...
export async function generateMetadata({
  ...
  try {
    const { articleSlug: pubSlug, slug: articleSlug } = await params;
...
async function ArticlePageContent({
  ...
  // outer [articleSlug] = publication slug, inner [slug] = article slug
  const { articleSlug: pubSlug, slug: articleSlug } = await params;

  if (process.env.NODE_ENV !== 'development') {
    redirect(`/${articleSlug}`);
  }
